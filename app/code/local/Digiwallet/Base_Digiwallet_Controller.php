...
class Base_Digiwallet_Controller extends Mage_Core_Controller_Front_Action
{
    ...
    private function execReport($orderId, $trxid)
    {
        ...
        $digiWallet = new DigiwalletCore($this->_tp_method, Mage::getStoreConfig("payment/$this->_code/rtlo"), $language, false);
        
        $digiWallet->checkPayment($trxid);
        
        $paymentStatus = (bool) $digiWallet->getPaidStatus();
        ...
        if ($paymentStatus) {
            ...
            if (!in_array($order->getState(), [Mage_Sales_Model_Order::STATE_PROCESSING, Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW])) {
                ...
                if ($this->_tp_method == 'BW' && $digiWallet->getBankwireAmountPaid() < $digiWallet->getBankwireAmountDue()) {
                    $invoice->setBaseGrandTotal($digiWallet->getBankwireAmountPaid() / 100);
                    $invoice->setGrandTotal($digiWallet->getBankwireAmountPaid() / 100);
                    $newState = Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW;
                    $orderGrandTotal = $digiWallet->getBankwireAmountPaid() / 100; // Make sure a Refund is not done for full amount if less was paid
                }
